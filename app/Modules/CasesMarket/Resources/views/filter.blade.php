        ...
        <form action="{{ route('filter_cases_market') }}" method="post">
            ...
            <div class="marketplace-sidebar">
                ...
                <div class="form-input">
                    <input placeholder="{{ transWord('Search Items') }}" type="text" id="items-search" @if(isset($request->items_search)) value="{{ $request->items_search }}" @endif name="items_search">
                  </div>
                  <br>
                      ...
                      @foreach (getCases() as $case)
                          <div class="checkbox-line">
                              <div class="checkbox-wrap">
                                ...
                                @if (is_array($request->case_name))
                                    @if (in_array($case,$request->case_name))
                                    <input type="checkbox" class="case_name" checked id="{{ $case }}" name="case_name[]" value="{{ $case }}">
                                    @else
                                    <input type="checkbox" class="case_name" id="{{ $case }}" name="case_name[]" value="{{ $case }}">
                                    @endif
                                @else
                                    @if ($case == $request->case_name[0])
                                    <input type="checkbox" class="case_name" checked id="{{ $case }}" name="case_name[]" value="{{ $case }}">
                                    @else
                                    <input type="checkbox" class="case_name" id="{{ $case }}" name="case_name[]" value="{{ $case }}">
                                    @endif

        ...
        <div class="card">
                ...
                <ul class="header-dropdown dropdown">
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <li><a href="{{ route('create_attachments') }}"><i class="icon-book-open"></i> {{ transWord('Create New Attachment') }}</a></li>
                            <li><a href="{{ route('attachments') }}"><i class="icon-book-open"></i> {{ transWord('All Attachment') }}</a></li>
                        </ul>
                    </li>
                </ul>
            </div>
            <div class="body">
                ...
                <ul class="nav nav-tabs3">
                    ...
                    @else
                    <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#Home-new2">{{ transWord('File') }}</a></li>
                    <li class="nav-item"><a class="nav-link show active" data-toggle="tab" href="#Profile-new2">{{ transWord('Link') }}</a></li>
                    @endif
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane show active" id="Home-new2">
                        ...
                        <form action="{{ route('update_attachments',Crypt::encrypt($attachment->id)) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                            {!! BuildFields('title' , getDataFromJson($attachment->title) , 'text' ,['required' => 'required']) !!}
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-lg-12">
                                    <label for="file">{{ transWord('File') }}</label>
                                    <input type="file" data-default-file="{{ asset('uploads/backend/attachments/'.$attachment->data) }}" name="file" id="file" class="dropify" required>
                                </div>
                            </div>
                    ...
                    @else
                    <div class="tab-pane" id="Home-new2">
                        <h6>{{ transWord('File') }}</h6>
                    ...
                    <div class="tab-pane show active" id="Profile-new2">
                        <h6>{{ transWord('Link') }}</h6>
                        <form action="{{ route('store_attachments') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <label for="url">{{ transWord('Link') }}</label>
                                    <input type="url" value="{{ $attachment->data }}" name="link" id="url" class="form-control" required placeholder="{{ transWord('Link') }}">
                                </div>
                            </div>
                            <input type="hidden" name="type" value="link">
                            <hr>

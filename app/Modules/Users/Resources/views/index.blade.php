    ...
    <div class="section-header">
        <div class="section-header-info">
          <h2 class="section-title" style="display: flex;">{{ $title }}
            @can('create_users')
            &nbsp;&nbsp;
            <a href="{{ route('create_users') }}" class="text-sticker">
      ...
      <div class="user-preview">
        ...
        <div class="user-preview-info">
          <div class="user-short-description">
            <a class="user-short-description-avatar user-avatar medium" href="profile-timeline.html">
              ...
              <div class="user-avatar-content">
                  @if ($user->hasRole('Admin'))
                    <div class="hexagon-image-82-90" data-src="{{ asset('dento/img/avatar/admin.png') }}" style="width: 82px; height: 90px; position: relative;"><canvas width="82" height="90" style="position: absolute; top: 0px; left: 0px;"></canvas></div>
                  @endif
            ...
            <p class="user-short-description-text">{{ transWord('username').' @'.$user->username }}</p>
            <p class="user-short-description-text">{{ transWord('email').' '.$user->email }}</p>
            <hr>
          ...
          <div class="user-preview-actions">
            ...
            @can('update_users')
            <a href="{{ route('edit_users',Crypt::encrypt($user->id)) }}" class="button secondary" title="{{ transWord('Update Information') }}">
            ...
            @can('delete_users')
                @if (Auth::user()->id != $user->id)
                    <a href="{{ route('delete_users',Crypt::encrypt($user->id)) }}" onclick="return confirm('{{ transWord('Are you sure?') }}')" class="button secondary" title="{{ transWord('Delete') }}">
            ...
            @can('permissions_users')
            <a href="{{ route('permissions_users',Crypt::encrypt($user->id)) }}" class="button secondary" title="{{ transWord('Permissions') }}">
